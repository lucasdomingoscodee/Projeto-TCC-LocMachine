...
// 1. Lógica: ADICIONAR AO CARRINHO (Sem sair da página)
...
// 2. Lógica: COMPRAR AGORA (Vai direto pro carrinho)
...
// 3. Lógica: SISTEMA DE BUSCA
$termo_busca = "";
$sql_busca = "SELECT * FROM maquinas"; // Padrão: traz tudo

if (isset($_GET['busca']) && !empty($_GET['busca'])) {
    $termo_busca = $_GET['busca'];
    // Filtra pelo nome usando LIKE
...
// Contagem do carrinho para o ícone
...
<body>
    ...
    <div class="container" id="catalogo">
        ...
        <div class="grid-maquinas">
            ...
            <?php foreach($maquinas as $index => $item): 
                // Pequeno truque para alternar o delay da animação
    ...
    <script>
        // 1. Verifica se a URL tem ?status=adicionado
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('status') === 'adicionado') {
            const toast = document.getElementById('toast');
            toast.classList.add('show');
            
            // Remove o parametro da URL sem recarregar (para ficar limpo)
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.style.animationPlayState = 'running'; // Garante que rode
                    entry.target.classList.add('visible'); // Opcional se usar classes diferentes
                }
            });
        });

        // Pega todos os elementos com a classe .reveal
